<template>
  <v-hover>
    ...
    >
      <v-flex>
        <v-list-tile-action>
          <v-img class="thumbnail" :src="thumbnail" :aspect-ratio="thumbnailSize">
...
export default {
  ...
  computed: {
    ...
    analyzesCount() {
      return this.$store.getters.getAnalyzesBySourceId(this.id).length
    },
    statusIcon() {
      let icon
      switch(this.status) {
        ...
        case 'DOWNLOADING':
          ...
          break;
        // Will be deprecated
        case 'UPLOADING TO GS':
        case 'UPLOADING_TO_GS':
          icon = 'upload';
          break;
        case 'DOWNLOADED':
          icon = 'check';
